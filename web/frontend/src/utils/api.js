...
export const adminSetSubscription = (username, action, plan) => api.post(`/api/admin/users/${username}/subscription`, { action, plan }).then(r => r.data)
export const adminDeleteUser = (username) => api.delete(`/api/admin/users/${username}`).then(r => r.data)
export const adminApprovePayment = (idx) => api.post(`/api/admin/payments/${idx}/approve`).then(r => r.data)
